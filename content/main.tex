...
\title{Mobile Security Final Report: \\ 
Making DidFail Succeed}

% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Jonathan Burket}
\IEEEauthorblockA{Andrew ID: jburket\\
jburket@cmu.edu}
\and
\IEEEauthorblockN{Jonathan Lim}
\IEEEauthorblockA{Andrew ID: jlim2\\
jlim2@andrew.cmu.edu}
\and
\IEEEauthorblockN{Wei Shen}
\IEEEauthorblockA{Andrew ID: wshen1\\
wshen1@andrew.cmu.edu}
\and
\IEEEauthorblockN{Will Snavely}
\IEEEauthorblockA{Andrew ID: wsnavely\\
...
DidFail's \emph{APK Transformer} identifies intent sinks in an Android app by matching method signatures in the app with known intent sink method signatures. If the signatures match, an Intent ID is generated for the intent sink method. Method signatures for the non-Activity intent sinks were added in order to track intent sinks for Broadcast Receivers and services. Similarly, FlowDroid had to be modified slightly to recognize the new Intent sinks. 

\subsubsection{Matching Flows by Component Types}
DidFail initially only handled flows involving activities. Extending DidFail to also support broadcast receivers and service components required a slight modification in how the sources and sinks were matched. Certain methods indicate the type of a receiving component, and need to be taken into account when matching receiving components and sending components $(e.g.\  \emph{startService} \rightarrow service, \emph{startActivity} \rightarrow activity).$ By recording the sink component type, we enabled the matching process to create appropriate flows.


\subsection{Initial Test Results}
We tested the analyzer on a single app set, with two apps that we created. \emph{SendApp}'s main activity obtains location information by calling \texttt{getLocation()} and stores the information in an intent set with a custom action string matching \emph{SendAppService}'s intent filter (See Figure~\ref{fig:servicetest1}). The intent is passed to \texttt{startService} and is received by \emph{SendApp}'s service, and similarly sent to the \emph{ReceiveAppService} component. \emph{ReceiveAppService} passes the tainted information to \emph{ReceiveApp}'s activity by calling \texttt{startActivity}.
...
\section{Incorporating Broadcast Receivers}

Currently, DidFail only detects flows via non-broadcast Intents. To extend DidFail for broadcast intents, there are a few things we needed to consider.  First, broadcast Intents are received in a particular way: the application contains a class that extends \texttt{android.content.BroadcastReceiver}, which implements the \texttt{onReceive} method.  Second, an application can declare the broadcasts it wishes to receive in two ways: statically in the manifest, or dynamically with the \texttt{registerReceiver()} method. 

Three modifications were required to make DidFail understand broadcast Intents: improving the Epicc parser to extract information about broadcast Intents, modifying FlowDroid to mark broadcast-related functions as sources/sinks, and extending Phase 2 to consume the new output resulting from the previous modifications.

\subsection{Parsing Epicc Output for Broadcast Receivers}
...
\caption{Example Epicc Output for a Broadcast Receiver}
\end{figure}

We added a separate parser named \emph{parse\_bcast} is added to \emph{epicc\_parser} to address this issue.
...
\begin{figure}[!h]
\lstinputlisting[language=Java]{broadcasttx.java}
\caption{Transmitter code leaking deviceID through a broadcast Intent}
\label{fig:broadcastleak}
\end{figure}

\begin{figure}[!h]
\lstinputlisting[language=XML]{broadcaststatic.xml}
\caption{Statically declared Broadcast Intent Receiver in a manifest file}
\label{fig:broadcastmanifest}
\end{figure}

\begin{figure}[!h]
\lstinputlisting[language=Java]{broadcastonreceive.java}
\caption{Code for a Broadcast Receiver that sends text messages}
\label{fig:broadcaststaticrecvr}
\end{figure}

\begin{figure}[!h]
\lstinputlisting[language=Java]{broadcastdyn.java}
\caption{Registering a Broadcast Receiver dynamically}
\label{fig:broadcastregister}
\end{figure}

\begin{figure}[!h]
\lstinputlisting[language=Java]{broadcastdynrcvr.java}
\caption{Code for a Broadcast Receiver that writes to a file}
...
Currently, in order for dynamically declared Broadcast Intent receivers to be identified as a sink/source, a dummy receiver declaration in the manifest file is manually added to make FlowDroid work. This is a known issue of FlowDroid and no easy short cut in FlowDroid code base was found to mitigate this. As a resort, we can modify the APK Transformer to add this dummy line into manifest file.
...
\begin{figure}
\lstinputlisting[language=Java]{appchat.java}
...
\begin{figure}
\lstinputlisting[language=Java, numbers=left]{aliased.java}
