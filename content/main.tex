...
A Service is an application component that is meant to run without direct user interaction (e.g provide some functionality while the app is not in the foreground). A Service can be instantiated by passing an intent corresponding to a Service's intent filter or the name of the class instance, and making a call to the \emph{startService} method. We added new functionality to DidFail to handle some \lori{I added word 'some' for multiple reasons. First, Service component analysis listed as a task in DHS proposal. Second and third reasons: Believe there are some which are missed. For instance, when arrays of intents are passed, flows are missed by DidFail, right? (not labelled by APK Transformer). Also, there was the problem with onActivityResult() analysis involving Service components glitching in FlowDroid.} dataflows involving Services.  
...
These initial enhancements moved us significantly closer to the goal of analyzing taint flows through all types of components and shared static fields.
%There are additional dataflows that need to be detected and analyzed, but these initial enhancements moved us significantly closer to the goal of analyzing all types of components and shared static fields.
