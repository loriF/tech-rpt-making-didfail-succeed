...
\item{} \emph{BadBroadcastRxDyn} is the same as \emph{BroadcastRx\_Dyn}, except it has different calls to the methods unregisterReceiver and registerReceiver.

\item{} \emph{KludgeBroadcastRxDyn} is the same as \emph{BadBroadcastRx\_Dyn}, except it has a filter declaration in the manifest as well as a standard dynamic registration of the BroadcastReceiver in the component’s Java code. See Figure~\ref{fig:broadcastDynManifestKludge} for the filter declaration code.
\end{itemize} 
...
Since FlowDroid was unable to statically detect flows in BroadcastRx\_Dyn, we tested this app on a physical phone, alongside BroadcastTx. Figure~\ref{fig:logcat-BcastDyn} shows the log messages (from \ttt{adb logcat}) when running BroadcastTx and BroadcastRx\_Dyn.
...
\end{alltt}
\caption{Output of \ttt{adb logcat}}
...
\TODO{Confirm what the \ttt{nopaths} option actually does.}
